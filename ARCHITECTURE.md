...
| GET | `/admin-preview/{token}` | admin.preview | `AdminPreviewController@show` | Превью несохранённой формы страницы/новости из админки (кнопка «Превʼю», открывается в slide-over-iframe): слепок состояния формы в кэше на 10 мин (`App\Support\AdminPreview`), рендер публичного шаблона без записи в БД, плашка «Попередній перегляд»; свежезагруженная обложка копируется в `public/admin-preview/`. Только залогиненным, иначе 404; путь добавлен в регексп исключений catch-all | нет |
